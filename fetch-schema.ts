...
const fetchSchema = async () => {
    ...
    try {
        ...
        fs.writeFileSync(schemaPath, JSON.stringify(response.properties, null, 2), "utf-8");
        console.log(`✅ Schema written successfully to ${schemaPath}`);
    } catch (error: any) {
        console.error("❌ Notion API Error:", error.message);
        if (error.body) {
             // Write error body to file if it's too large
             fs.writeFileSync(path.resolve(__dirname, "fetch_error.json"), JSON.stringify(error.body, null, 2), "utf-8");
