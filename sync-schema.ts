...
const PROPERTY_KEY_MAP: Record<string, string> = {
  ...
  "If approved, how soon can you actively work and test a new lead list?": "speed",
  ...
  "Are you willing to pay for a small pilot to validate lead quality before scaling": "paidPilot",
...
const MANUAL_DESCRIPTIONS: Record<string, string> = {
  ...
  "Email": "Your best email to receive follow-ups, reports, and onboarding details.",
  "Phone": "Optional   if you'd like us to text or call you with updates or appointment confirmations.",
  "Business Type / Niche": "“What specific niche does your business operate in?” Example: wholesaling, Airbnb, botanicals, SaaS apps, e-commerce, event management, coaching, etc.",
  "ICP Category": "Who is your ideal customer? The type of people or businesses you aim to serve or sell to.",
  "Describe Your Ideal Customer Profile": "Tell us who your ideal customer is. Describe the type of person or business you want to attract, serve, or sell to including demographics, behavior, buying intent, and any specific traits that define a perfect-fit client",
  "High-Intent Lead Sources": "“List any websites, URLs, or platforms you believe would have high-value leads for your business. These are the places you’d want us to scrape, monitor, or extract leads from.” Examples: Zillow links, MLS boards, competitor sites, directory pages, Google Maps, LinkedIn searches, etc.",
  "Avg Deal Amount ($)": "“On average, how much do you earn per deal, appointment, or closed customer?” Used to estimate ROI and automation impact.",
  "Deals / Month": "“How many deals or clients do you typically close each month?” This helps us estimate your sales velocity.",
  "Lead Volume / Month": "“How many new leads do you want generated monthly across all channels?” Useful for scoring lead flow and automation capacity.",
  "Conversion Rate %": "What percent of leads do you convert",
  "Current CRM": "“What CRM or system do you use today to manage your leads?” Examples: HubSpot, Zoho, GHL, Lofty, spreadsheets, none.",
  "Existing Lead Lists?": "“Do you already have spreadsheets or contact lists you'd like us to clean, enrich, or import?”",
  "Main Pain Point(s)": "“What’s the biggest challenge you want solved right now?” Examples: lead quality, inconsistent appointments, too much manual work, bad data, etc.",
  ...
  "Start Date ": "What date would you like to begin? Please enter your ideal start date for onboarding, setup, or automation work.",
  "Detailed Scraping Instructions ?": "“Please describe exactly what you want us to scrape. Include any URLs, keywords, filters, pages, or specific data points you want captured.”",
  "Interested in these features?": "Select any features you’re interested in using. This helps us tailor your setup, automation, and pricing to the tools that matter most for your business.",
  "Are you willing to pay for a small pilot to validate lead quality before scaling": "Paid pilots help us ensure high-quality data tailored to your needs.",
  "Schedule Kickoff Call": "If you qualify when can you hop on a kick off call to meet your start date deadlines? Book here: https://calendar.notion.so/meet/cyberoni/em2w42l93",
  "Notes": "Add any additional context, goals, or details about your business we should know.",
  "Company Website ?": "Your primary business website URL.",
  "Who will be responsible for reviewing and acting on the leads?": "Select the primary person or system handling these leads.",
  "If approved, how soon can you actively work and test a new lead list?": "This helps us schedule your onboarding timeline.",
  "What is your expectation for initial validation?": "Define what a successful pilot looks like for you.",
  "Can we connect directly to your CRM?": "This allows for seamless lead handoff."
};

// field order definition
const FIELD_ORDER = [
  // 1. Essentials
  " Name", "Email", "Phone", "Company Website ?", "Business Type / Niche",
  // 2. Qualification
  "Monthly Budget", "Main Pain Point(s)", "Start Date ", "Priority Level",
  // 3. Scope
  "ICP Category", "Describe Your Ideal Customer Profile", "High-Intent Lead Sources", "Detailed Scraping Instructions ?",
  // 4. Details
  "Lead Volume / Month", "Deals / Month", "Avg Deal Amount ($)", "Conversion Rate %",
  // 5. Logistics
  "Current CRM", "Can we connect directly to your CRM?", "Existing Lead Lists?",
  "Who will be responsible for reviewing and acting on the leads?",
  "What is your expectation for initial validation?",
  "If approved, how soon can you actively work and test a new lead list?",
  "Are you willing to pay for a small pilot to validate lead quality before scaling",
  // 6. Closing
  "Interested in these features?", "Notes", "Schedule Kickoff Call"
];
  ...
  try {
    ...
    try {
        ...
        if ((db as any).data_sources && (db as any).data_sources.length > 0) {
            ...
            console.log(`ℹ️  Provided ID is a View. Attempting to resolve Source ID: ${realDbId}`);
            try {
                db = await notion.databases.retrieve({ database_id: realDbId });
                properties = db.properties;
            } catch (e: any) {
                console.warn(`⚠️  Could not access Source Database (${realDbId}). Missing permissions?`);
                console.warn(`   Falling back to Page Inspection. Select Options will be empty.`);
            }
        } else {
             properties = db.properties;
        }
    ...
    if (!properties) {
        console.log(`ℹ️  Falling back to querying the database to infer schema from pages...`);
        console.log(`ℹ️  Will also collect select/multiselect options from all pages.`);
        ...
        if (query.results && query.results.length > 0) {
             ...
             console.log(`ℹ️  Scanning ${query.results.length} pages to collect select options...`);
             for (const page of query.results) {
                 ...
                 for (const [propName, propValue] of Object.entries(pageProps)) {
                     ...
                     if (pv.type === 'select' && pv.select?.name) {
                         if (!collectedOptions.has(propName)) collectedOptions.set(propName, new Set());
             ...
             console.log(`✅ Collected options for ${collectedOptions.size} select/multiselect properties`);
        } else {
             throw new Error("Could not retrieve Source Database AND Database is empty. Cannot infer schema.");
        }
    }

    if (!properties) { // Should be caught above
        throw new Error("Could not retrieve properties.");
    }

    console.log(`✅ Properties retrieved` + (usingFallback ? " (via Page Inspection)" : " (via Database Schema)"));
    ...
    const remainingKeys = allKeys.filter(key => !FIELD_ORDER.includes(key)).sort();
    ...
    for (const key of finalSortedKeys) {
      ...
      if (propType === "select" || propType === "multi_select") {
        ...
        if (MANUAL_OPTIONS_OVERRIDE[key]) {
            ...
            console.log(`  → Using ${options.length} options for "${key}" from MANUAL_OPTIONS_OVERRIDE`);
        }
        ...
        if ((!options || options.length === 0) && collectedOptions.has(key)) {
            ...
            if (collected.length > 0) {
                ...
                console.log(`  → Using ${collected.length} options for "${key}" from page scan`);
            }
        }
        
        if (options && options.length > 0) {
            const varName = `${camelName}Options`;
            optionsCode += `const ${varName} = ${JSON.stringify(options.map((o: any) => ({
                value: o.name,
      ...
      if (camelName === "name") zodDefinition = 'z.string().min(2, "Name is required")';
      if (camelName === "email") zodDefinition = 'z.string().email("Invalid email address")';
...
`;
    ...
    // Write to file
    const outputPath = path.resolve(__dirname, "../src/data/contact/intakeFormFields.ts");
    ...
    fs.writeFileSync("sync_debug.log", JSON.stringify(error, Object.getOwnPropertyNames(error), 2));
  }
}
