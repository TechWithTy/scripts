...
if (!pkg.scripts["test:rspack"]) {
	const testCmd = pkg.scripts["test:vitest"]
		? "pnpm run test:vitest"
		: hasVitest
			? "vitest run"
			: "pnpm test";
	pkg.scripts["test:rspack"] =
...
if (
	pkg.scripts["build:rspack"] &&
	typeof pkg.scripts["build:rspack"] === "string"
) {
	if (
		pkg.scripts["build:rspack"].includes(
